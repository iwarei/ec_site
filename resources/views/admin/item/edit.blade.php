    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <div>
        <div class="flex items-center gap-x-3">
          <h2 class="text-lg font-medium text-gray-800 dark:text-white">アイテム編集</h2>
        </div>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">編集するアイテム情報を入力してください。</p>
      </div>
    </div>
    <form action="{{ route('admin.item.update', $item) }}" method="post">
      @csrf
      @method('put')
